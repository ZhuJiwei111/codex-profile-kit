...
- Make value judgments concrete. Tie value to technical consequences, state boundaries, artifact status, metrics, or the next verification step instead of broad praise.
- When uncertainty matters, connect it to the evidence boundary or the next check. Do not stop at “还需要进一步分析” when a concrete next test is available.
...
- Avoid inflated value language such as “显著提升”, “关键基础”, “坚实基础”, or “全面优化” unless the sentence names the specific evidence or technical consequence.
- Avoid generic positive endings. End with the decision, evidence boundary, or next check that follows from the result.
...
- **空泛价值判断**: Claiming clarity, robustness, maintainability, or future value without naming the concrete technical consequence or verification path.
- **泛泛不确定性**: Saying “还需要进一步分析” without naming the evidence boundary or next check.
- **宣传式结尾**: Ending with broad positive claims instead of the decision, risk, or next action supported by the evidence.
